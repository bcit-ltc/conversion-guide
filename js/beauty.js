/* Via: http://code.google.com/p/burp-suite-beautifier-extension/source/browse/trunk/src/burp/JSBeautifier/js/beautify-html.js?r=3 */
function style_html(t, e) {
	function i() {
		return this.pos = 0, this.token = "", this.current_mode = "CONTENT", this.tags = {
			...
			var n, s = e.join("");
			n = s.indexOf(-1 != s.indexOf(" ") ? " " : ">");
			var h = s.substring(1, n).toLowerCase();
			if ("/" === s.charAt(s.length - 2) || this.Utils.in_array(h, this.Utils.single_token)) this.tag_type = "SINGLE";
			else if ("script" === h) this.record_tag(h), this.tag_type = "SCRIPT";
			else if ("style" === h) this.record_tag(h), this.tag_type = "STYLE";
			else if (this.Utils.in_array(h, unformatted)) {
				var r = this.get_unformatted("</" + h + ">", s);
				e.push(r), this.tag_type = "SINGLE"
			} else if ("!" === h.charAt(0))
				if (-1 != h.indexOf("[if")) {
					if (-1 != s.indexOf("!IE")) {
						var r = this.get_unformatted("-->", s);
						e.push(r)
					}
					this.tag_type = "START"
				} else if (-1 != h.indexOf("[endif")) this.tag_type = "END", this.unindent();
			else if (-1 != h.indexOf("[cdata[")) {
				var r = this.get_unformatted("]]>", s);
				e.push(r), this.tag_type = "SINGLE"
			} else {
				var r = this.get_unformatted("-->", s);
				e.push(r), this.tag_type = "SINGLE"
			} else "/" === h.charAt(0) ? (this.retrieve_tag(h.substring(1)), this.tag_type = "END") : (this.record_tag(h), this.tag_type = "START"), this.Utils.in_array(h, this.Utils.extra_liners) && this.print_newline(!0, this.output);
			return e.join("")
		...
		}, this.printer = function (t, e, i, n, s) {
			this.input = t || "", this.output = [], this.indent_character = e, this.indent_string = "", this.indent_size = i, this.brace_style = s, this.indent_level = 0, this.max_char = n, this.line_char_count = 0;
	...
	var n, s, h, r, a;
	for (e = e || {}, s = e.indent_size || 4, h = e.indent_char || " ", a = e.brace_style || "collapse", r = 0 == e.max_char ? 1 / 0 : e.max_char || 70, unformatted = e.unformatted || ["a"], n = new i, n.printer(t, h, s, r, a);;) {
		var _ = n.get_token();
		if (n.token_text = _[0], n.token_type = _[1], "TK_EOF" === n.token_type) break;
		switch (n.token_type) {
		case "TK_TAG_START":
		...
		case "TK_TAG_END":
			if ("TK_CONTENT" === n.last_token && "" === n.last_text) {
				var o = n.token_text.match(/\w+/)[0],
					u = n.output[n.output.length - 1].match(/<\s*(\w+)/);
				(null === u || u[1] !== o) && n.print_newline(!0, n.output)
			}
		...
		case "TK_SCRIPT":
			if ("" !== n.token_text) {
				n.output.push("\n");
				var l = n.token_text;
				if ("TK_SCRIPT" == n.token_type) var c = "function" == typeof js_beautify && js_beautify;
				else if ("TK_STYLE" == n.token_type) var c = "function" == typeof css_beautify && css_beautify;
				if ("keep" == e.indent_scripts) var p = 0;
				else if ("separate" == e.indent_scripts) var p = -n.indent_level;
				else var p = 1;
				var g = n.get_full_indent(p);
				if (c) l = c(l.replace(/^\s*/, g), e);
				else {
					var f = l.match(/^\s*/)[0],
						d = f.match(/[^\n\r]*$/)[0].split(n.indent_string).length - 1,
						T = n.get_full_indent(p - d);
					l = l.replace(/^\s*/, g).replace(/\r\n|\r|\n/g, "\n" + T).replace(/\s*$/, "")
				}
				l && (n.print_token(l), n.print_newline(!0, n.output))
			}
			n.current_mode = "TAG"
		}
